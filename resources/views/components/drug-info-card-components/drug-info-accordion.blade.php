            ...
            <div class="accordion-body">
                <ul class="list-group list-group-flush">
                    <li class="list-group-item"><span
                            class="fw-bold">Medication Status:</span> {{ $drug->products[0]->marketing_status }}</li>

                    {{-- determine if there is more than one ingredient, use blade component if so --}}
                    @if (count($drug->products[0]->active_ingredients) == 1)

                        <li class="list-group-item"><span
                                class="fw-bold">Active Ingredients:</span> {{ $drug->products[0]->active_ingredients[0]->name . ' - ' . $drug->products[0]->active_ingredients[0]->strength}}
                        </li>

                    @else
                        <x-drug-info-card-components.active-ingredients
                            :ingredients="$drug->products[0]->active_ingredients"/>

                    @endif

                    <li class="list-group-item"><span
                            class="fw-bold">Dosage Form:</span> {{ $drug->products[0]->dosage_form }}</li>
                    <li class="list-group-item"><span class="fw-bold">Intake:</span> {{ $drug->products[0]->route }}
                    </li>
                </ul>
            </div>
        </div>
